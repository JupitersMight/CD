# -*- coding: utf-8 -*-
"""
Created on Thu Nov 15 19:59:48 2018
...
DATA_PATH = 'C:\\Users\\joao-\\OneDrive\\Documentos\\Ciencia de Dados\\Project\\CD\\datasets\\aps_test_average.csv'
...
y = df.iloc[:,0:2].values#(columns=['consensus'])
...
for col in list(df) :
    df[col] = pd.cut(df[col],5,labels=range(1,6))#, duplicates='drop')
...
with pd.option_context('display.max_rows', 10, 'display.max_columns', 10): 
    (rules.sort_values(by=['support','lift','leverage'],ascending =False).to_csv(path_or_buf='Colposcopy\\aps_rules2.csv'))
...
for index, row in r.iterrows():
    if float(row['support'])>0.2:
        lifts_2.append(row['support'])
    if row['support']>0.3:
    ...
    if row['support']>0.9:
        lifts_9.append(row['lift'])
    if row['support']==1:
        lifts_10.append(row['lift'])
...
print (sum(lifts_9)/len(lifts_9))
#print (sum(lifts_10)/len(lifts_10)) 
print()
